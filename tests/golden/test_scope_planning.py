...
from skfd.core.lir import ConstDecl, VarDecl, Axiom, Theorem, ScopeEnter, ScopeExit, FloatingHyp
...
def test_golden_scope_planning_determinism() -> None:
    ...
    unit = ProofUnitIR(
        ...
        lir_stmts=[
            ...
            FloatingHyp(label=l_wffph, typecode=c_wff, var=v_ph, stmt_id=4, origin_ref=orig),
            Axiom(label=l_ax1, typecode=c_wff, expr=(v_ph,), stmt_id=5, origin_ref=orig),
            Theorem(label=l_th1, typecode=c_wff, expr=(v_ph,), proof=(l_ax1,), stmt_id=6, origin_ref=orig),
...
"""
    ...
    assert output == expected_corrected, f"Output mismatch!\nGot:\n{output}\nExpected:\n{expected_corrected}"

...
## 14. Generated Tree V1 发布边界（2026-07-22）

`catalog-compiler` 现拥有来源中立的 `generated-tree-v1` content/record contract、
显式 generated-output policy，以及可收敛到 terminal `COMPLETE` 的 selected POSIX
publisher。该边界不把 Set.mm 或 proof-scaffold 策略移入通用 compiler core。

确定性 tree 只记录获授权的逻辑 owned root、精确 portable file inventory、byte facts、
content digest 与生成 context。Host root、inode/device facts、lock、staging name、
journal 与 publication receipt 都是 runtime-only，并位于 generated tree 外。Deployment
必须显式选择 target/control root、requested-content provider、atomic adapter、durability
行为与 pinned receipt schema。

Retained-lock session 在 staging 前安装 durable journal，独立验证 frozen bytes，使用
真正的 exclusive install 或 atomic directory exchange，并到达 `DURABLE` visibility/
durability boundary。其无参完成操作内部构造并存储 publication receipt，独立 reopen，
重验 exact post topology，在 transaction-owned deletion 前记录 `CLEANED`，证明 durable
staging absence，最后记录 `COMPLETE`。Terminal head 保留到不同事务真正安装 origin；
永久 receipt 阻止 transaction ID 重用。

这改变的是 proof-scaffold 的集成义务，而不是其权威。未来 semantic-package backend
只能把 frozen generated bytes 与显式 capability descriptor 提供给 selected publisher。
现有 direct write、`rmtree`、authoring-project 与 legacy transpiler 路径不是该 capability
的包装，不得称为 atomic publication。

`COMPLETE` 是 Generated Tree V1 的运行结果。它不校验 Manifest V3，不证明 declaration
coverage，不建立 trust/foundation closure，不比较 frame，不验证 proof，也不构成独立
verification receipt。它同样不会把当前 partial Set.mm catalog 变成 production release：
仍需 complete publishable lock、snapshot-matched dependency graph、已裁决 provider
layout、精确 Prelude/provider locks 与独立验证。

该边界验收门为：

| 门 | 要求 |
| --- | --- |
| GTB1 | 通用 tree、policy、journal、receipt 与 publisher contract 不含 Set.mm、Python、Prelude、proof format 或固定 generated-root 默认值。 |
| GTB2 | Publisher 只能替换显式获授权 owned root；unknown topology、link、extra file、receipt drift 与 ambiguous recovery 全部 preserve-all。 |
| GTB3 | `DURABLE` 仍是 atomic visibility/durability boundary；只有 receipt-bound exact cleanup 与 absence proof 才可推进到 `COMPLETE`。 |

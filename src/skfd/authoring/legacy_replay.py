...
from .ids import AssertionSemanticId, JudgmentKindId, SortId
...
from .replay import SemanticReplayPlan
from .term import Term, VariableRef

LegacyReplayOperation = Literal["ref", "apply"]


@dataclass(frozen=True, slots=True, kw_only=True)
class LegacyAssertionReplayBinding:
    assertion: AssertionSemanticId
    backend_label: str
    operation: LegacyReplayOperation
    legacy_rule: str | None = None

    def __post_init__(self) -> None:
        if self.operation not in ("ref", "apply"):
            raise AuthoringSemanticError(
                f"unsupported legacy replay operation: {self.operation}"
            )
        if not self.backend_label:
            raise AuthoringSemanticError("legacy assertion label must be non-empty")
        if self.operation == "apply" and self.legacy_rule != "mp":
            raise AuthoringSemanticError("legacy replay only supports the mp apply rule")
        if self.operation == "ref" and self.legacy_rule is not None:
            raise AuthoringSemanticError("legacy ref binding cannot override its rule name")
...
class LegacyReplayBinding:
    ...
    assertions: tuple[LegacyAssertionReplayBinding, ...]
    ...
    def __post_init__(self) -> None:
        by_assertion: dict[AssertionSemanticId, LegacyAssertionReplayBinding] = {}
        for assertion in self.assertions:
            if assertion.assertion in by_assertion:
                raise AuthoringSemanticError(
                    f"duplicate legacy assertion binding: {assertion.assertion}"
                )
    ...
    def lower_judgment(self, judgment: Judgment) -> Wff:
        if judgment.kind != self.provable_judgment or len(judgment.arguments) != 1:
            raise AuthoringSemanticError("legacy proof lowering requires one provable term")
        formula = self.lower_term(judgment.arguments[0])
        if formula.sort != "wff":
            raise AuthoringSemanticError("legacy proof conclusion must lower to wff")
        return cast(Wff, formula)


def lower_semantic_replay_plan(
    plan: SemanticReplayPlan,
    binding: LegacyReplayBinding,
    *,
    proof_name: str,
) -> Proof:
    if not proof_name:
        raise AuthoringSemanticError("legacy proof name must be non-empty")
    final_position = len(plan.hypotheses) + len(plan.applications) - 1
    if plan.root_position != final_position:
        raise AuthoringSemanticError(
            "legacy proof lowering requires the root to be the final position"
        )
    ...
    for application in plan.applications:
        ...
        try:
            ...
            raise AuthoringSemanticError(
                f"legacy replay references unknown position: {error.args[0]}"
            ) from error
        steps.append(
            Step(
    ...
    if root is None:
        raise AuthoringSemanticError("legacy replay root position is unknown")
    statement = binding.lower_judgment(plan.signature.conclusion)
    if binding.lower_judgment(root) != statement:
        raise AuthoringSemanticError("legacy replay root does not match its statement")
    try:
        ...
        raise AuthoringSemanticError(
            f"invalid legacy replay distinct-variable relation: {error}"
        ) from error

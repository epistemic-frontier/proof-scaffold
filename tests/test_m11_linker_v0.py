...
def test_adv_m11_forbid_raw_string_tokens_default_off() -> None:
    ...
    lir = [
        ...
        LIRTheorem(
            ...
            typecode=1,
            expr=(2, 5, 4, 5, 3),
            proof_tokens=(6, "ax-id"),  # type: ignore[arg-type]
            origin=Origin(module="mod", file="x.py", line=5),
        ),
    ]
    u = ProofUnitIR(
        unit_id="adv.raw",
        lir=cast(list[LIRStmt], lir),
        origin=Origin(module="mod", file="x.py", line=0),
        symtab=("wff", "|-", "(", ")", "->", "ph", "wph"),
    )
    try:
        LinkerV0().link([u])
        ...
        assert "raw string" in msg or "non-int" in msg or "forbidden" in msg
        assert "x.py" in msg
...
def test_adv_m11_forbid_cross_unit_hyp_leakage() -> None:
    ...
    ub = ProofUnitIR(
        ...
        symtab=("wff", "(", ")", "->", "ph", "ps", "wph", "wps", "h1", "h2", "h1r"),
    )
    try:
        LinkerV0().link([ua, ub])
    ...
    ua = ProofUnitIR(
        ...
        lir=[
            ...
            LIRTheorem(
                ...
                typecode=0,
                expr=(1,),
                proof_tokens=(2,),  # self-ref bogus, we only care about visibility
                origin=Origin(file="A.py", line=4),
            ),
            ScopeExit(origin=Origin(file="A.py", line=5)),
        ],
        origin=Origin(file="A.py", line=0),
        symtab=("wff", "ph", "secret_thm"),
    ...
    ub = ProofUnitIR(
        ...
        lir=[
            ...
            LIRTheorem(
                ...
                typecode=0,
                expr=(1,),
                proof_tokens=(2,),  # attempt to use A's non-exported label

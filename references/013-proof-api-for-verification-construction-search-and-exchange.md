# Reference 013：面向验证、证明建构、搜索与交换的 Proof API
...
当前 API 已经是一套不错的**进程内、完整证明建构 API**：typed `Term`、
`AssertionSignature`、统一 assertion application kernel、`ProofAuthor.use()/qed()`、显式
catalog/profile 和 DV 检查都应保留。

它还不是验证、搜索和交换共同依赖的 proof protocol。最大的缺口不是再增加一种表达能力，
而是边界没有闭合：

```text
对象能够构造并拥有 content digest
                  ≠
对象已在锁定的 theory、依赖与信任策略下完成验证
```

`ElaboratedProof` 构造时进行的是 proof graph 的结构检查；真正按照 calculus、catalog 和 profile
重新应用每一步 assertion 的检查，目前发生在 `build_semantic_replay_plan()`。与此同时，现有
内部 digest 投影虽有版本标签，却没有公开且规范的 proof/theory artifact codec、严格 decoder、
wire schema 或完整 theory lock。
...
或可持久分支。普通线性证明不受影响，但交互建构与搜索没有合适的公共状态边界。

下一阶段应采用以下裁决：

1. **先把现有完整证明提升为可独立重放、可交换的证书。** 不另造一套与
   `ElaboratedProof` 平行的 Proof DAG 类。
2. **需要交互建构和搜索时，只增加一个最小、不可变的 `ProofState`。** 不把 hole 或
   metavariable 塞进 verifier 的 hole-free `Term` 或最终 proof。
3. **三层是内部不变量边界，不是三套普通用户 API。** Execution 层先复用现有 replay lowering
   和 Metamath artifact；没有性能证据前，不设计新的 Proof VM、二进制 ISA 或硬件 packet。
4. **Package / provenance 先做薄封套和 sidecar。** 不立即引入一套庞大的、可编辑的通用
   Package IR。
...
四种能力分别是建构（`prove/refine`）、验证（`verify`）、在 `ProofState` 上进行外部搜索，以及
交换（`load/save`）。`ProofAuthor` 继续是默认 façade，`ProofState` 只向需要它的角色渐进披露。
普通用户不应被要求理解 catalog digest、profile lock、packet、Merkle node、execution plan、
...
这里的 `THEORY.prove(...)` 只是把当前 `ProofAuthor(...)` 所需的 language/calculus/catalog/profile
...
proof = THEORY.load("mp2b.skir")  # decode + verify，默认 fail closed
```

以上名称是说明性的，尚未冻结；关键是普通用户只操作 `Theory` 和 `Proof`。未验证 packet、codec
limits、dependency lock 和 replay plan 都是内部或 advanced API。happy-path `save` 在写出前验证，
`load` 在返回前验证。load 时的验证只是返回 `Proof` 的门槛，不会把“已验证”状态写进 proof
identity；需要审计证据时另取 `VerificationReport` 或 certificate。
...
dependencies，新的 artifact identity 不应把这些派生值当作第二份权威输入。实现可以缓存或
展示它们，但必须重新计算；缓存不得改变新的 proof identity。当前 v2 `semantic_digest` 尚未遵循
这一新投影，兼容处理见 6.3 节。
...
```text
LanguageInterface + CalculusInterface + AssertionCatalog/Profile
                              │
                              v
                         ProofAuthor
                              │ use()
...
4. `ElaboratedProof` immutable、root-reachable、无 hole，只包含 concrete assertion applications；
   family/combinator 已在 elaboration 前消失。
5. `build_semantic_replay_plan()` 已经能够逐步从 profile/catalog 解析 assertion，再以记录的
   premises、target 和 substitution 重新调用 public application kernel。
6. language、calculus、catalog 和 proof 已有版本化 digest 投影，为规范 artifact 奠定了基础。
7. 四领域 transpilation 已证明这条 complete-proof happy path 能处理真实规模语料，并已有生成与
   generated-artifact import/re-elaboration 基线；逐 assertion replay、transitive closure 和独立
   Metamath verifier 仍须按第 7 节分项建立基线。

因此，下一步不是推翻 authoring API，而是把已经存在的语义内核放到正确的公开边界后面。
...
- 函数名 `build_semantic_replay_plan` 把最重要的验证边界隐藏成了 lowering 准备步骤。
...
当前 proof 投影记录 `calculus_digest` 和 assertion nominal ID，但没有逐 assertion interface digest，
也没有精确 theory/import lock。nominal ID 说明“它叫什么”，不能单独说明“这个名字对应哪份
...
3. 引入只读 `Theory` / `TheoryLock`，分别绑定 language、calculus、assertion interfaces、imports、
   assertion profile 和 `TrustPolicy`。profile 只限制“哪些 assertion 可以应用”；它不能授予
   theorem 无证明可信性。
4. `Theory.verify_all()` 按 dependency DAG 验证所有 theorem implementations。默认 trust roots
   ...
   implementation。外部 theorem 只能来自 digest 匹配的 verified dependency artifact，或被高级
...
verification_digest
  = H(implementation_digest + theory lock + assertion profile
      + trust policy
      + verification protocol version)
```

同一个 proof DAG 在不同审计策略下仍是同一实现，但不是同一份验证结论。
`verification_digest` 只是 result identity/cache key，不等于验证过程本身、数字签名或可脱离 report
使用的 certificate。

### 3.3 现有交换物是可执行 Python，不是 proof artifact

当前 canonical helper 只做 `Mapping -> JSON bytes -> SHA-256`。内部投影虽有 version 字段，仍没有
公开、规范的 artifact canonical bytes、严格 decoder、wire schema 或 accept/reject vectors。
生成包通过 import Python 模块执行 `prove_*()` 来重建 `PROOFS`；这对受信构建流程可用，却不是
跨组织、不可信输入或长期存档的边界。

第一版交换格式不需要通用二进制。严格 canonical JSON 足够作为第一版规范编码：

下面的 `TheoryLock` 和 `ProofArtifactV1` 是 advanced wire concepts，不计入普通作者的概念预算：
...
ProofArtifactV1
  ...
  root position
  optional non-semantic provenance sidecar
```

`result`、`satisfied_distinct` 和 direct dependencies 由 verifier 产生，不是 packet 的权威事实。
若为调试或速度携带缓存值，verifier 必须重算并核对，且缓存不进入 implementation identity。
...
- unknown version、unknown/missing field、duplicate key 和 malformed ID fail closed；
...
`TheoryLock` 只锁定内容要求，不会凭空提供内容。self-contained artifact 必须嵌入验证所需的
interface/implementation closure；thin artifact 必须通过 content-addressed resolver 取得精确内容。
resolver 缺项或 digest 不符时一律 fail closed。

完整 theory 交换可用同一封套携带 interfaces、import locks、trust-root declarations 和 theorem
implementations。单 theorem artifact 是对 dependency closure 的切片，不需要发明另一种 proof
...
它没有 goal、hole、metavariable 或 constraint，也没有稳定的搜索状态 identity。对同一个 immutable
...
`ProofAuthor` 另外用 Python object identity 判断 step 是否属于当前 author；这是简洁、安全的线性
façade，但它本身不能持久化或跨进程分支。
...
- 内部锁定 Theory/profile 引用，并保存 metavariables 与 equality/sort/DV constraints；
...
verifier replay 和 `ProofState.finish()` 的最终 acceptance 必须共享它。backward `refine()` 另由
constraint-producing elaborator 产生 metas 与 residual constraints；partial application 闭合后必须
reify，并通过同一 concrete checker 才能进入最终 proof。两者可共享 term matching、substitution 和
DV primitives，但不能假装 incomplete refinement 与 concrete acceptance 是同一条执行路径。
...
|---|---|---|---|
| 验证 | application kernel、replay、calculus/catalog/profile digest | 构造/digest 与 verified 状态混淆；theorem dependency closure 未验证 | `Theory.verify()`、`verify_all()`、结构化 report、精确 lock |
| 证明建构 | `ProofAuthor.use()/qed()` 的低 ceremony happy path | 环境参数过多；无真正 partial state | `Theory.prove()` 绑定环境；高级用户才使用 `ProofState` |
| 搜索 | typed signature、函数式 draft branch、同一最终 acceptance semantics | 分支复制/重扫成本高；无 goal/refine、锁定 state identity 和 machine-readable failure | 复用 `ProofState.refine()`；frontier/score 留给搜索器 |
| 交换 | stable nominal IDs、versioned internal digest projection | 无公开 wire schema/codec/decoder 和闭合 theory lock；必须执行 producer Python | `Theory.load/save` + strict canonical artifact，load 必须验证 |
...
已缓存依赖后的增量 closure，或输出 lowering/Metamath evidence；默认行为必须安全且无需用户组装
calculus、catalog 和 profile。
...
首版只承诺 canonical snapshot bytes 相同就有相同 `snapshot_digest`。它是保守的 snapshot identity，
不承诺跨 lineage 的数学状态等价或 search dedup；等价状态完全可以有不同摘要。alpha-equivalence、
proof equivalence 和 aggressive normalization 在真实 corpus 数据证明收益前，不进入公共身份契约。

第一阶段只需从 Theory 派生按 conclusion judgment kind、head constructor 和 premise count 的只读
候选索引。它是可重建的内部索引或 search sidecar，不进入 semantic catalog/digest。embedding、
历史频率和 model ranking 同样只是 digest-linked advisory sidecar。
...
- `TheoryLock`：解释并验证它所需的精确接口与政策；
- artifact envelope：schema、可选 dependency payload 与非语义 metadata。
...
self-contained artifact 随文件提供 closure；thin artifact 则要求调用方的 resolver 按 digest 提供
closure。仅有 `TheoryLock` 但无法解析其内容时，loader 必须拒绝。

第一版只承诺 ProofScaffold semantic model 与 Metamath lowering 的无损 round trip。Lean、Coq、
...
| Execution projection | 无推断、引用已解析、线性；`SemanticReplayPlan` 仍 backend-neutral，binding 后的 legacy proof / `.mm` 才 backend-specific | 否；先复用现有两段路径 |

与三者正交的也不是一个庞大的第四 IR，而是两个薄对象：

```text
TheoryInterface / TheoryLock
  提供 semantic environment 与精确依赖要求
...
Theory ────────> complete Proof ────────> verify / save
   │                  │
   │                  v
   │          internal replay/lowering ─────> .mm / verifiers
   │
...
这里没有用户手工执行“Construction IR lowering 到 Proof DAG，再 lowering 到 Execution IR”的
...
### 6.1 分开 nominal identity 与 content compatibility

- language、sort、constructor、assertion、theory 使用稳定、可读的 nominal ID；
- interface digest 表示同一 nominal ID 的精确内容要求；
- theorem signature 的 `interface_digest` 与 proof body 的 `implementation_digest` 分开；
- assertion profile 与 trust policy 分别进入 verification digest，不改变相同 proof DAG 的
  implementation identity；
- source path、作者、模型、时间、note、rendering 和 backend token 不进入 proof identity。

### 6.2 StepRef 与 snapshot 不是全球数学身份

proof-local canonical positions 足以作为 v1 artifact 的引用。当前没有必要把每个 step 变成全球
...
不需要永久全球唯一。`snapshot_digest` 只摘要 exact canonical snapshot，不承担跨 lineage dedup。

### 6.3 摘要只能证明内容相同

任何 content digest 都不能代替 verifier result。API 与文档不得用 `semantic_digest` 暗示“已经
验证”。兼容期保留 legacy `semantic_digest`；新的 versioned `implementation_digest` 使用不同的
domain/projection，加入精确 assertion interface requirements，并可排除可推导缓存。因此它不承诺
等于旧值，也不能只是旧字段的 alias。迁移期 verification report 可以同时携带二者，并明确
`theory_lock_digest` 与 `verification_digest`。
...
1. Python/generated artifact import 或 JSON decode；
...
6. lowering 与独立 Metamath verifier；
7. warm incremental verification；
8. scalar `refine` throughput，以及 batch API 引入后的 batch throughput；
9. artifact encode/decode throughput、bytes 和 peak RSS。

每次基准必须记录：source commit、ProofScaffold/transpiler commit、artifact digest、Python/runtime、
...
lowering 是主瓶颈后才进入设计。性能目标先由数据给出，不以“硬件未来可能需要”为理由冻结 ISA。
...
### 8.1 共同前置：稳定 acceptance boundary

1. 增加只读 `Theory` façade/lock，统一绑定 language、calculus、catalog、assertion profile、
...
2. 从当前 application 代码抽出不依赖 `ProofDraft` 和位置 StepId 的 concrete checker，并定义稳定
   ...
   acceptance boundary。
3. 从 `build_semantic_replay_plan()` 提升出 public `verify_proof()`；lowering 消费已验证 replay，
   不再让 replay-plan builder 兼任唯一验证入口。

这三项稳定后，验证/交换与 construction/search 可以并行推进；后者不必等待整个 artifact 工作完成。
...
3. **A3，data artifact：** 统一 Term/Judgment/Signature/Proof canonical projection，实现 strict
...
1. 在共同 checker 周围增加 constraint-producing elaborator，以及最小 `ProofState`、opaque
...
3. `ProofState` 使用 lineage-scoped value refs；`finish/save` 时把 root-reachable steps 规范化为
   proof-local canonical positions，artifact 只保存这些位置引用。`ProofAuthor` 可以继续用 Python
   object handle 做安全、低 ceremony 的 façade ownership check；现有 happy path 不必接受任意
   外部 `StepRef`。
4. 让 `ProofAuthor` 的最终 acceptance 复用共同 checker，保证生成代码无需改写。
...
- streaming/NDJSON 或紧凑 binary artifact；
...
- verifier 能在新进程中不 import producer Python package 而检查 artifact。self-contained artifact
  自带 closure；thin artifact 的 resolver 缺项或摘要不符时拒绝。
...
- provenance/source map 改变不影响 implementation digest；proof body 改变不影响 theorem
  interface digest；assertion profile、trust policy 与 verification result 不混入 proof identity。
...
- import/decode、structure/digest、semantic replay、theory closure、lowering、independent verifier 和
...
- 现在部署全球 Merkle store 或承诺 proof/alpha equivalence 的 canonical identity；
...
验证、建构、搜索与交换四类边界一旦成立，其余场景应作为派生能力，而不是反过来塑造 v1 核心：
...
- IDE、LLM trajectory、candidate validation 和 curriculum 从 `ProofState` transition 与 structured
  diagnostics 派生；
- publication、citation、provenance、narrative 和 source map 从 artifact sidecars 派生；
- distributed frontier、GPU/FPGA packet 和 hardware execution 从已验证 Proof 的 lowering 派生；
- knowledge graph、proof mining 和历史分析从 proof/theory artifact 的只读 projection 派生。

最终的架构原则不是“用一个扁平 IR 做所有事情”，也不是“为每个场景建立一套 IR”，而是：

> 用一个 concrete assertion-application acceptance kernel 和一个 constraint elaborator，维护
> partial state、complete proof 与 execution projection 三种互不混淆的不变量；再用一个极小
> façade，让每类用户只看到自己需要的入口。
...
简单地写出 proof
    -> 在精确 theory 下确定性验证
    -> 保存为不执行 producer Python 的版本化纯数据 artifact
    -> 在另一个进程或实现中加载并重新验证
```

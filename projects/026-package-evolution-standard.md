...
### 2.1 prelude 内容标准（已裁决，细化于 Project 027）

定性边界已由用户裁决，规范细化见
[Project 027：Prelude 边界 RFC](027-prelude-boundary-rfc.md)：

- prelude 的边界是**通用理论构造能力**（表示原语、组合机制），
  不是"常用数学内容"；到集合/类基础、关系与函数为止，贴近
  set.mm 原生分层——**自然数（含 ω）、有限性、归纳、有限
  递归、有限序列/fold 均不入 prelude**，归集论与数领域
  （2026-07-19 二次裁决，实证依据见 027 §4.1 后注）；

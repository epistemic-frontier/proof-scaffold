...
> Status: Draft v0.4, 2026-07-19 (Prelude boundary retreated by second adjudication: natural numbers stay out; the prelude stops at relations and functions).  
...
| Prelude | 前导包 | The globally, implicitly visible base layer inside a release package, bounded by **general theory-building capability** (representation primitives and combination mechanisms, up to set/class basics and relations/functions, close to set.mm's native layering) and carrying no substantive theory of any specific mathematical domain; natural numbers (including ω), finiteness, induction, and finite recursion stay out of the prelude. | Boundary principle and the second adjudication in the 027 RFC; the quantitative boundary awaits empirical work. Semantic foundations belong to the prelude; authoring economy belongs to the Python layer. |

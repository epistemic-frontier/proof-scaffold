...
class FrameStack(list):
    ...
    def add_d(self, stat):
        frame = self[-1]
        frame.d.update(
            ((min(x, y), max(x, y)) for x, y in itertools.product(stat, stat) if x != y)
        )
    ...
    def make_assertion(self, stat):
        ...
        mand_vars = {
            tok
            for hyp in itertools.chain(e_hyps, [stat])
            for tok in hyp
        ...
        dvs = {
            (x, y)
            for fr in self
            for (x, y) in fr.d.intersection(itertools.product(mand_vars, mand_vars))
        }
            ...
            if steptyp in ("$a", "$p"):
                ...
                for h in hyp:
                    ...
                    if entry != subst_h:
                        raise MMError(
                            (
                                "stack entry {0!s} doesn't match " + "hypothesis {1!s}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Proof verifier for the Metamath language"
    )

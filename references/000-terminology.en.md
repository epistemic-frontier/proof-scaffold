...
| Provider layout | 实现提供者布局 | A versioned, content-addressed physical implementation projection that binds governed declarations to public surfaces, the selected implementation closure, provider shards, target artifacts, typed entry points, and the exact direct cross-shard dependency quotient. | Keeps public ownership separate from replaceable implementation placement. It is an explicit semantic-package input, not catalog authority, proof-order inference, or a compiler-core default. |

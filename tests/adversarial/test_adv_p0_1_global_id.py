...
def test_adv_p0_1_global_id_relocation() -> None:
    ...
    c_a = interner.intern(
        origin_module_id="A", local_name="c", kind="Const", origin_ref=orig_a
    )
    unit_a = ProofUnitIR(
        unit_id="A:unit",
        origin_ref=orig_a,
    ...
    c_b = interner.intern(
        origin_module_id="B", local_name="c", kind="Const", origin_ref=orig_b
    )
    unit_b = ProofUnitIR(
        unit_id="B:unit",
        origin_ref=orig_b,
    ...
    assert (
        len(tokens) == 2
    ), f"Expected 2 constants emitted, got {len(tokens)}: {tokens}"
